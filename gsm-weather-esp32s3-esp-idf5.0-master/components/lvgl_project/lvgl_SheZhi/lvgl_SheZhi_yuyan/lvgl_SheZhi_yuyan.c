...
const char*SheZhi_yuyan_biaoti_TEXT[][SYSTEM_LANGUAGE_NUM] =
{
	{ "语言", "WIFI", "WIFI", NULL},

};

const char*SheZhi_yuyan_TEXT[][SYSTEM_LANGUAGE_NUM] =
{
...
void lvgl_SheZhi_yuyan_button_cb(struct _lv_obj_t * obj, lv_event_t event)
{
	...
	case LV_EVENT_CLICKED://非拖动下释放
		...
		lvgl_SheZhi_yuyan_XuanZhong(lvgl_SheZhi_yuyan_GongNeng_Data.AnXia_button);
	break;
...
//----选中时的背景----//
		...
		for (i = 0; i < SheZhi_yuyan_Button_Num; i++)
		{
			...
			lv_label_set_text(lvgl_SheZhi_yuyan_Label[i], (const char*)SheZhi_yuyan_TEXT[i][system_get_Language()]);
	...
	lvgl_SheZhi_yuyan_XiaLa_Jin();

	lvgl_SheZhi_yuyan_task = lvgl_task_create(lvgl_SheZhi_yuyan_TaskCb, 100, LV_TASK_PRIO_LOWEST, NULL);
...
const char * lvgl_SheZhi_yuyan_get_zhuangtai_str(void)
{
	...
	return (const char*)SheZhi_yuyan_TEXT[system_get_Language()][system_get_Language()];
}
