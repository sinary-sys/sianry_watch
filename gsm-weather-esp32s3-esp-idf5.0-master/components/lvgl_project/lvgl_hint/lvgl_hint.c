...
const char*tishi_TEXT[][SYSTEM_LANGUAGE_NUM] =
{
	{ "提示", "提示", "tishi", NULL},
};
...
void lvgl_hint_create(lv_obj_t * Fu,char *text,uint8_t touming,uint8_t shijian)
{
	...
	if (lvgl_tishi_main_cont == NULL)
	{
		...
		if(tishi_ChuangKou.lvgl_task ==NULL)
			tishi_ChuangKou.lvgl_task = lvgl_task_create(lvgl_tishi_TaskCb, 1000, LV_TASK_PRIO_LOWEST, NULL);
		tishi_DEBUG("创建窗口\r\n");
	}
	else
	{
